...
export default function NoteDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      {loading && (
        <p className="text-[#8A9BB0] text-sm">Loading note...</p>
      )}
      ...
      {!loading && !error && note && (
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between gap-4">
            <h1 className="text-2xl font-semibold tracking-tight text-[#F5ECD7] flex-1">
              {note.title}
            </h1>
          ...
          <div className="flex items-center gap-4 text-xs text-[#4A5A6A]">
            <div className="flex items-center gap-1.5">
              ...
              <span>
                Created {format(new Date(note.created_at), "MMMM d, yyyy")}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5" />
              <span>
                Updated {format(new Date(note.updated_at), "MMMM d, yyyy 'at' h:mm a")}
              </span>
            </div>
          </div>
